...
class ModelSnapshot:
    ...
    @classmethod
    def create(cls, graph: CanonicalBipartiteGraph, affinity: sparse.csr_matrix, embeddings: np.ndarray, assignments: np.ndarray, neighbor_ids: np.ndarray, neighbor_scores: np.ndarray, config: BuildConfig, diagnostics: tuple[dict[str, float], ...]) -> "ModelSnapshot":
        return cls(str(uuid4()), datetime.now(UTC).isoformat(), graph, affinity.tocsr(), embeddings, assignments, neighbor_ids, neighbor_scores, config, diagnostics)
...
class SnapshotStore:
    ...
    def load(self, snapshot_id: str) -> ModelSnapshot:
        ...
        graph = CanonicalBipartiteGraph(tuple(manifest["u_ids"]), tuple(manifest["v_ids"]), sparse.load_npz(target / "A.npz").tocsr(), sparse.load_npz(target / "X.npz").tocsr())
        ...
        return ModelSnapshot(manifest["snapshot_id"], manifest["created_at"], graph, sparse.load_npz(target / "W.npz").tocsr(), np.load(target / "Z.npy", allow_pickle=False), np.load(target / "S.npy", allow_pickle=False), np.load(target / "semantic_neighbors.npy", allow_pickle=False), np.load(target / "semantic_scores.npy", allow_pickle=False), config, tuple(manifest["diagnostics"]))
